...
SpeedTracker.prototype.getConfig = function (force) {
  ...
  return this._getRemoteFile('speedtracker.yml').then(data => {
    try {
      ...
      return Promise.reject(Utils.buildError('INVALID_CONFIG1'))
    }
  }).catch(err => {
    const str = 'INVALID_CONFIG2:' + JSON.stringify(err)
    // console.log(err)
    return Promise.reject(Utils.buildError(err))
  })
}
...
SpeedTracker.prototype.getRandomKey = function (str) {
  const arr = str.split('|')
  const ret = arr[Math.floor(Math.random() * arr.length)]
  console.log('getRandomKey: ', ret)
  return ret
}
...
SpeedTracker.prototype.initWpt = function (profile) {
  ...
  let wptKey = this.config.wptKey ? Utils.decrypt(this.config.wptKey, this.options.key) : this.getRandomKey(config.get('wpt.key'))
...
SpeedTracker.prototype.runTest = function (profile, isScheduled) {
  ...
  return this.initConfig().then(() => {
    ...
    return this.options.scheduler.find(profileData._nwo, profileData._id).then(schedule => {
      ...
      if (runTest) {
        // parameters.location = 'Dulles:Chrome'
        // parameters.connectivity = '3G'
        console.log('\r\n runTest parameters:\r\n', parameters)
        testJob = this._runWptTest(url, parameters, wpt => {
          // console.log(3333333333333333, arguments)
          return this._getPagespeedScore(url).then(score => {
            console.log('\r\n _getPagespeedScore(url, score):', url, score)
